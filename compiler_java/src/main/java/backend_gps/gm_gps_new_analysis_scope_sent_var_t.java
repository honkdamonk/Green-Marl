...
public class gm_gps_new_analysis_scope_sent_var_t extends gm_apply {
	...
	private gm_gps_new_scope_analysis current_scope = gm_gps_new_scope_analysis.GPS_NEW_SCOPE_GLOBAL;
	...
	public final boolean apply(gm_symtab_entry e, symtab_types symtab_type) {
		...
		case GPS_NEW_SCOPE_GLOBAL:
			s = gm_gps_scope.GPS_SCOPE_GLOBAL;
			break;
		case GPS_NEW_SCOPE_IN:
			s = gm_gps_scope.GPS_SCOPE_INNER;
			break;
		case GPS_NEW_SCOPE_EDGE:
		case GPS_NEW_SCOPE_RANDOM:
		case GPS_NEW_SCOPE_OUT:
		default:
			s = gm_gps_scope.GPS_SCOPE_OUTER;
			break;
		}
	...
	public final boolean apply(ast_sent s) {
		...
		if (s.get_nodetype() == ast_node_type.AST_FOREACH) {
			...
			if (outer_loop == null) {
				...
				assert (fe.get_iterator().getTypeSummary() == gm_type.GMTYPE_NODEITER_NBRS)
			...
			if (a.is_target_scalar() && a.get_lhs_scala().getTypeInfo().is_edge_compatible()) {
				if (current_scope == gm_gps_new_scope_analysis.GPS_NEW_SCOPE_IN) {
					// check rhs
					...
					if ((b.get_driver().getSymInfo().find_info_bool(GPS_FLAG_IS_INNER_LOOP))
							&& (b.get_builtin_def().get_method_id() == gm_method_id.GM_BLTIN_NODE_TO_EDGE)) {
