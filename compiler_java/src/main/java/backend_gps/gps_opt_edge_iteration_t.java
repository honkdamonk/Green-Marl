...
public class gps_opt_edge_iteration_t extends gm_apply {
	...
	public final void post_process() {
		...
		for (I = _targets.iterator(); I.hasNext();) {
			...
			ast_foreach outer_fe = gm_new_sents_after_tc.gm_new_foreach_after_tc(outer_id, fe.get_iterator().getTypeInfo().get_target_graph_id().copy(true),
					sb, gm_type.GMTYPE_NODEITER_ALL);
			...
			gm_symtab_entry new_edge_symbol = gm_add_symbol.gm_add_new_symbol_nodeedge_type(sb2, gm_type.GMTYPE_EDGE, fe.get_iterator().getTypeInfo()
					.get_target_graph_sym(), old_edge_iter_name);
			...
			ast_expr rhs = ast_expr_builtin.new_builtin_expr(inner_id.copy(true),
					gm_main.BUILT_IN.find_builtin_def(gm_type.GMTYPE_NODEITER_NBRS, gm_method_id.GM_BLTIN_NODE_TO_EDGE), null);
