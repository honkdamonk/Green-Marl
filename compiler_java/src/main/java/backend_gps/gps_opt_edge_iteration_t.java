...
public class gps_opt_edge_iteration_t extends gm_apply
{
	...
	public boolean apply(ast_sent s)
	{
		if (s.get_nodetype() == ast_node_type.AST_FOREACH)
		{
			...
			if (fe.get_iter_type() == gm_type.GMTYPE_EDGEITER_ALL)
			{
				_targets.addLast(fe);
			}
		}
	...
	public final void post_process()
	{
		...
		for (I = _targets.iterator(); I.hasNext();)
		{
			...
			ast_foreach outer_fe = gm_new_sents_after_tc.gm_new_foreach_after_tc(outer_id, fe.get_iterator().getTypeInfo().get_target_graph_id().copy(true), sb, gm_type.GMTYPE_NODEITER_ALL);
			...
			ast_foreach inner_fe = gm_new_sents_after_tc.gm_new_foreach_after_tc(inner_id, outer_id.copy(true), body, gm_type.GMTYPE_NODEITER_NBRS);
			...
			gm_symtab_entry new_edge_symbol = gm_add_symbol.gm_add_new_symbol_nodeedge_type(sb2, gm_type.GMTYPE_EDGE, fe.get_iterator().getTypeInfo().get_target_graph_sym(), old_edge_iter_name);
			...
			ast_expr rhs = ast_expr_builtin.new_builtin_expr(inner_id.copy(true), gm_main.BUILT_IN.find_builtin_def(gm_type.GMTYPE_NODEITER_NBRS, gm_method_id.GM_BLTIN_NODE_TO_EDGE), null);
