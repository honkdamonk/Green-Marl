...
public class gps_opt_find_updown_foreach_t extends gm_apply {
	...
	public final void post_process() {
		...
		for (ast_foreach fe : targets) {
			...
			ast_expr check_level = ast_expr.new_comp_expr(gm_ops.GMOP_EQ,
					ast_expr.new_field_expr(ast_field.new_field(fe.get_iterator().copy(true), lev_sym.getId().copy(true))),
					ast_expr.new_biop_expr(op_for_check, ast_expr.new_id_expr(curr_sym.getId().copy(true)), ast_expr.new_ival_expr(1)));
