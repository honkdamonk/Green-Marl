	...
	public boolean apply(ast_expr e) {
		if (e.is_builtin()) {
			...
			if (b.get_driver().getSymInfo() == sym) {
				...
				Pair<ast_sentblock, gm_method_id> P = new Pair<ast_sentblock, gm_method_id>(sb, b.get_builtin_def().get_method_id());
				gm_symtab_entry target = null;
				if (!map.containsKey(P)) {
					String temp_name = gm_main.FE.voca_temp_name_and_add("tmp");
					gm_type type = b.get_builtin_def().get_result_type_summary();
				...
				e.set_expr_class(gm_expr_class.GMEXPR_ID);
