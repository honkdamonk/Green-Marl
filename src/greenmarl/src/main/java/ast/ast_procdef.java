...
public class ast_procdef extends ast_node
{
	...
	public void reproduce(int ind_level)
	{
		...
		for (it = in_args.begin(); it.hasNext(); it++, cnt++)
		{
			(it.next()).reproduce(ind_level);
			if (cnt != (last - 1))
				Out.push(",\n");
		}
		...
		if (last > 0)
		{
			Out.push(";\n");
			cnt = 0;
			for (it = out_args.begin(); it.hasNext(); it++, cnt++)
			{
				(it.next()).reproduce(ind_level);
				if (cnt != (last - 1))
					Out.push(",\n");
	...
	public void dump_tree(int ind_level)
	{
		...
		System.out.print(" IN:\n");
		java.util.Iterator<ast_argdecl> it;
		for (it = in_args.begin(); it.hasNext();)
		{
			(it.next()).dump_tree(ind_level + 1);
			System.out.print("\n");
		}
		System.out.print("\n");
		GlobalMembersGm_dumptree.IND(ind_level);
		System.out.print(" OUT:\n");
		for (it = out_args.begin(); it.hasNext();)
		{
			(it.next()).dump_tree(ind_level + 1);
			System.out.print("\n");
		}
