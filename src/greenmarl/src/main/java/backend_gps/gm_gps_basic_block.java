...
//C++ TO JAVA CONVERTER NOTE: Java does not allow default values for parameters. Overloaded methods are inserted above.
//ORIGINAL LINE: void add_exit(gm_gps_basic_block* b, boolean add_reverse = true)
	...
	{
		System.out.print("[--------------------\n");
		System.out.printf("%d (%s):\n", id, (type == gm_gps_bbtype_t.GM_GPS_BBTYPE_SEQ) ? "SEQ" : (type == gm_gps_bbtype_t.GM_GPS_BBTYPE_IF_COND) ? "IF" : (type == gm_gps_bbtype_t.GM_GPS_BBTYPE_WHILE_COND) ? "WHILE" : (type == gm_gps_bbtype_t.GM_GPS_BBTYPE_BEGIN_VERTEX) ? "PAR" : "???");
		//printf("\tnum_entries:%d\n", entries.size());
		System.out.print("\t[ ");
		for (int i = 0; i < (int) entries.size(); i++)
		{
			System.out.printf("%d ", entries[i].get_id());
		}
		System.out.print("]=>...\n");
		...
		for (int i = 0; i < (int) exits.size(); i++)
		{
			System.out.printf("%d ", exits[i].get_id());
		}
		System.out.print("]\n");
	...
	public void reproduce_sents(boolean reproduce_receiver)
	{
		...
		else if ((type == gm_gps_bbtype_t.GM_GPS_BBTYPE_PREPARE1) || (type == gm_gps_bbtype_t.GM_GPS_BBTYPE_PREPARE2) || (type == gm_gps_bbtype_t.GM_GPS_BBTYPE_MERGED_TAIL) || (type == gm_gps_bbtype_t.GM_GPS_BBTYPE_MERGED_IF))
		{
			// do nothing;
		}
		else
...
//C++ TO JAVA CONVERTER NOTE: Java does not allow default values for parameters. Overloaded methods are inserted above.
//ORIGINAL LINE: void add_info_ptr(String id, Object* ptr1, Object* ptr2 = null)
