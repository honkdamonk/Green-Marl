...
public class GlobalMembersGm_expand_group_assignment
{
	...
	public static ast_foreach create_surrounding_fe(ast_assign a)
	{
		...
		int iter;
		if (GlobalMembersGm_defs.gm_is_node_property_type(second.getTypeSummary()))
			iter = GMTYPE_T.GMTYPE_NODEITER_ALL.getValue();
		else if (GlobalMembersGm_defs.gm_is_edge_property_type(second.getTypeSummary()))
			iter = GMTYPE_T.GMTYPE_EDGEITER_ALL.getValue();
		else
			assert false;
