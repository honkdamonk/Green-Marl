...
//C++ TO JAVA CONVERTER NOTE: Java has no need of forward class declarations:
	...
	//extern void gm_set_curr_procname(tangible.RefObject<String> c); // for error print
...
//C++ TO JAVA CONVERTER NOTE: 'extern' variable declarations are not required in Java:
	...
	// return: is_okay
	//--------------------------------------------------------
	//extern boolean gm_apply_compiler_stage(java.util.LinkedList<gm_compile_step> LIST);
...
//C++ TO JAVA CONVERTER NOTE: 'extern' variable declarations are not required in Java:
	...
	public static ast_node GM_expr_biop(ast_node left, ast_node right, int op, int l, int c)
	{
	...
	public static ast_node GM_expr_uop(ast_node left, int op, int l, int c)
	{
	...
	public static ast_node GM_expr_lbiop(ast_node left, ast_node right, int op, int l, int c)
	{
	...
	public static ast_node GM_expr_luop(ast_node left, int op, int l, int c)
	{
	...
	public static ast_node GM_expr_comp(ast_node left, ast_node right, int op, int l, int c)
	{
	...
	public static ast_node GM_expr_reduceop(int op, ast_node iter, ast_node src, int iter_op, ast_node body, ast_node filter, ast_node src2, int l, int c)
	{
		...
		assert GlobalMembersGm_defs.gm_is_iter_type(iter_op);
		assert (op == GM_REDUCE_T.GMREDUCE_MAX.getValue()) || (op == GM_REDUCE_T.GMREDUCE_MIN.getValue()) || (op == GM_REDUCE_T.GMREDUCE_PLUS.getValue()) || (op == GM_REDUCE_T.GMREDUCE_MULT.getValue()) || (op == GM_REDUCE_T.GMREDUCE_AND.getValue()) || (op == GM_REDUCE_T.GMREDUCE_OR.getValue()) || (op == GM_REDUCE_T.GMREDUCE_AVG.getValue());
	...
	// type declarations
	public static ast_node GM_graphtype_ref(int graph_type_id)
	{
		return ast_typedecl.new_graphtype(graph_type_id);
	}

	public static ast_node GM_primtype_ref(int prim_type_id)
	{
		return ast_typedecl.new_primtype(prim_type_id);
	}

	public static ast_node GM_settype_ref(int set_type_id, ast_node id)
	{
		if (id != null)
			assert id.get_nodetype() == AST_NODE_TYPE.AST_ID;
	...
	public static ast_node GM_reduce_assign(ast_node lhs, ast_node rhs, ast_node id, int reduce_type)
	{
		assert rhs.is_expr();
		if (id != null)
			assert id.get_nodetype() == AST_NODE_TYPE.AST_ID;
	...
	public static ast_node GM_argminmax_assign(ast_node lhs, ast_node rhs, ast_node id, int reduce_type, lhs_list l_list, expr_list r_list)
	{
		assert rhs.is_expr();
		if (id != null)
			assert id.get_nodetype() == AST_NODE_TYPE.AST_ID;
	...
	public static ast_node GM_foreach(ast_node id, ast_node source, int iter_typ, ast_node sent, ast_node filter, boolean is_seq, boolean is_backward, ast_node source2)
	{
		assert id.get_nodetype() == AST_NODE_TYPE.AST_ID;
		...
		assert GlobalMembersGm_defs.gm_is_iter_type(iter_typ);
		ast_id i = (ast_id) id;
	...
	public static ast_node GM_bfs(ast_node it, ast_node source, ast_node root, ast_node navigator, ast_node f_filter, ast_node b_filter, ast_node f_sent, ast_node b_sent, boolean use_tp, boolean is_bfs)

	{
		...
		ast_bfs bfs = ast_bfs.new_bfs((ast_id) it, (ast_id) source, (ast_id) root, (ast_expr) navigator, (ast_expr) f_filter, (ast_expr) b_filter, (ast_sentblock) f_sent, (ast_sentblock) b_sent, use_tp, is_bfs);
