...
//C++ TO JAVA CONVERTER NOTE: 'extern' variable declarations are not required in Java:
	//extern java.util.HashMap<gm_symtab_entry*, range_cond_t> Default_DriverMap;
	//extern void traverse_expr_for_readset_adding(ast_expr e, java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> rset, java.util.HashMap<gm_symtab_entry, range_cond_t> DrvMap);
	public static boolean is_reported(java.util.LinkedList<conf_info_t> errors, gm_symtab_entry t, gm_symtab_entry b, int y)
	{
	...
	public static void add_report(java.util.LinkedList<conf_info_t> errors, gm_symtab_entry t, gm_symtab_entry b, int y)
	{
	...
	public static int check_leveled_access(gm_rwinfo e1)
	{
		...
		if (a_range == gm_range_type_t.GM_RANGE_LEVEL.getValue())
			lev = 1;
		else if (a_range == gm_range_type_t.GM_RANGE_LEVEL_UP.getValue())
			lev = 2;
		else if (a_range == gm_range_type_t.GM_RANGE_LEVEL_DOWN.getValue())
			lev = 0;

		return lev;
	}

	public static boolean check_if_conflict(java.util.LinkedList<gm_rwinfo> l1, java.util.LinkedList<gm_rwinfo> l2, gm_rwinfo e1, gm_rwinfo e2, int conf_type)
	{
		...
		for (i1 = l1.iterator(); i1.hasNext();)
		{
			...
			for (i2 = l2.iterator(); i2.hasNext();)
			{
				...
				if ((conf_type == gm_conflict_t.RW_CONFLICT.getValue()) || (conf_type == gm_conflict_t.WW_CONFLICT.getValue()) || (conf_type == gm_conflict_t.RM_CONFLICT.getValue()) || (conf_type == gm_conflict_t.WM_CONFLICT.getValue()))
				{
				...
				{
					if (e2.reduce_op == GM_REDUCE_T.GMREDUCE_DEFER.getValue())
						continue;
	...
	// return is_okay.
	//------------------------------------------------------------
	public static boolean check_rw_conf_error(java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> S1, java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> S2, int conf_type, java.util.LinkedList<conf_info_t> Report)
	{
		...
		java.util.Iterator<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> i1;
		java.util.Iterator<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> i2;
		for (i1 = S1.iterator(); i1.hasNext();)
		{
			gm_symtab_entry sym1 = i1.next().getKey();
			java.util.LinkedList<gm_rwinfo> list1 = i1.next().getValue();
			gm_rwinfo e1 = null;
			...
			for (i2 = S2.iterator(); i2.hasNext();)
			{
				...
				java.util.LinkedList<gm_rwinfo> list2 = i2.next().getValue();
				gm_rwinfo e2 = null;
	...
	//==================================================================
	public static boolean gm_does_intersect(java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> S1, java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> S2, boolean regard_mutate_direction)
	{
...
//ORIGINAL LINE: java.util.HashMap<gm_symtab_entry*, java.util.LinkedList<gm_rwinfo*>*>& Q_M = Q_SET->mutate_set;
	...
	public static java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> gm_get_reduce_set(ast_sent S)
	{
		assert S != null;
		return GlobalMembersGm_rw_analysis.get_rwinfo_sets(S).reduce_set;
	}
	public static java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> gm_get_write_set(ast_sent S)
	{
		assert S != null;
	...
	public static boolean gm_is_modified_with_condition(ast_sent S, gm_symtab_entry e, gm_rwinfo_query Q)
	{
		...
		for (i = W.iterator(); i.hasNext();)
		{
			...
			// find exact match
			java.util.LinkedList<gm_rwinfo> list = i.next().getValue();
	...
	// For debug
	//-----------------------------------------------------
	public static void gm_print_rwinfo_set(java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> m)
	{
