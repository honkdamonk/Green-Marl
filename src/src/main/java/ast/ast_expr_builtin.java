...
public class ast_expr_builtin extends ast_expr
{
	...
	public void reproduce(int ind_level)
	{
		...
		int size = args.size();
		Out.push('(');
		for (I = args.begin(); I.hasNext(); I++, cnt++)
		{
			(I.next()).reproduce(0);
			if (cnt != (size - 1))
				Out.push(',');
	...
	public void dump_tree(int ind_level)
	{
		...
		if (args.size() > 0)
		{
			System.out.print("\n");
			java.util.Iterator<ast_expr> I;
			for (I = args.begin(); I.hasNext();)
			{
				(I.next()).dump_tree(ind_level + 1);
				System.out.print("\n");
			}
			GlobalMembersGm_dumptree.IND(ind_level);
			System.out.print(")]");
	...
	public static ast_expr_builtin new_builtin_expr(ast_id id, String orgname, expr_list t)
	{
		...
		if (t != null)
		{
			E.args = new java.util.LinkedList<ast_expr>(t.LIST); // shallow copy LIST
			// but not set 'up' pointer. 
			java.util.Iterator<ast_expr> I;
			for (I = E.args.iterator(); I.hasNext();)
				(I.next()).set_parent(E);
			if (t != null)
				t.dispose(); // t is only temporary, delete it.
		}
		return E;
	}
