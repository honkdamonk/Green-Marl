...
public class ast_expr_builtin_field extends ast_expr_builtin
{
	...
	public static ast_expr_builtin_field new_builtin_field_expr(ast_field field, String orgname, expr_list exList)
	{
		...
		if (exList != null)
		{
			newExpression.args = new java.util.LinkedList<ast_expr>(exList.LIST); // shallow copy LIST
			// but not set 'up' pointer.
			java.util.Iterator<ast_expr> iter;
			for (iter = newExpression.args.iterator(); iter.hasNext();)
				(iter.next()).set_parent(newExpression);
			if (exList != null)
				exList.dispose(); // t is only temporary, delete it.
		}
