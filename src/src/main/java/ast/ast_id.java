...
//C++ TO JAVA CONVERTER NOTE: Java does not allow default values for parameters. Overloaded methods are inserted above.
//ORIGINAL LINE: void setSymInfo(gm_symtab_entry* e, boolean is_symtab_entry = false)
	...
	public final void set_orgname(String c) // copy is saved. old name is deleted
	{
		if (name != null)
			name = null;
		name = new byte[c.length() + 1];
		name = c;
	}
	...
	public final void set_genname(String c) // copy is saved. old name is deleted
	{
		if (gen_name != null)
			gen_name = null;
		gen_name = new byte[c.length() + 1];
		gen_name = c;
	}
