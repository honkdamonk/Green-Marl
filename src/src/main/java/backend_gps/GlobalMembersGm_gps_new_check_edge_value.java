...
public class GlobalMembersGm_gps_new_check_edge_value {
	...
	//
	// [1] Conditions
	// - Access to edge property is available only through edge variable that is
	// defined
	// inside the 2nd-level FE.
	// - Edge variable, defined inside a 2nd-level FE, should be initialized as
	// (2nd-level iterator).toEdge.
	...
	// e.A = e.B + n.Y; // okay. n.Y or e.B is not transported, but locally
	// used.
	// e.B = s.Y; // error
	// } }
	//
	...
	// GPS_MAP_EDGE_PROP_ACCESS : <where:>foreach, <what:>
	// map(symbol,int(state)), one of GPS_ENUM_EDGE_VALUE_xxx
	// GPS_FLAG_EDGE_DEFINED_INNER: <where:>var symbol(edge type),<what:>if the
	// varaible is defined inside inner loop
	// GPS_FLAG_EDGE_DEFINING_INNTER: <where:>foreach, <what:>if this inner loop
	// defines an edge variable
	// GPS_LIST_EDGE_PROP_WRITE: <where:>foreach, <what:> (list of) assigns
	// whose target is edge variables
	// GPS_FLAG_EDGE_DEFINING_WRITING:<where:>assign, <what:>if this assignment
	// is defining en edge (as inner.ToEdge())
	// ------------------------------------------------------------------------------------
	...
	public static boolean manage_edge_prop_access_state(ast_foreach fe, gm_symtab_entry e, int op) {
		...
		// ORIGINAL LINE: int* curr_state = (int*)
