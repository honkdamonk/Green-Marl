...
public class GlobalMembersGm_gps_opt_split_loops_for_flipping {
	...
	// extern void gm_gps_find_double_nested_loops(ast_node p,
	// HashMap<ast_foreach, ast_foreach> MAP);
	...
	public static void filter_target_loops(HashMap<ast_foreach, ast_foreach> SRC, HashSet<ast_foreach> SET) {
		for (ast_foreach in : SRC.keySet()) {
			...
			gm_rwinfo_map DMAP = GlobalMembersGm_rw_analysis_check2.gm_get_reduce_set(in.get_body());
	...
	public static void ensure_no_dependency_via_scala(LinkedList<ast_node> frame, HashMap<ast_sentblock, LinkedList<ast_sent>> elder,
			HashMap<ast_sentblock, LinkedList<ast_sent>> younger) {
		// create prev/next
		// create set of scalar
		HashSet<gm_symtab_entry> PREVS = new HashSet<gm_symtab_entry>(); // symbols
																			// used
																			// in
																			// prev
		HashSet<gm_symtab_entry> NEXTS = new HashSet<gm_symtab_entry>(); // symbols
																			// used
																			// in
																			// next
		HashSet<gm_symtab_entry> ALL = new HashSet<gm_symtab_entry>(); // every
																		// scalar
																		// symbols
		for (ast_node node : frame) {
