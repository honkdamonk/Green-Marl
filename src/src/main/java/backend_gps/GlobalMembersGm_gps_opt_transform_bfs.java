...
//ORIGINAL LINE: sbyte* i_name = FE.voca_temp_name_and_add("i", null, true);
		byte i_name = GlobalMembersGm_main.FE.voca_temp_name_and_add("i", null, true);
	...
	public static ast_sentblock create_fw_body_prepare(ast_sentblock while_sb, ast_bfs bfs, gm_symtab_entry lev_sym, gm_symtab_entry curr_sym, ast_foreach out)
	{
		...
		ast_foreach foreach_out = GlobalMembersGm_new_sents_after_tc.gm_new_foreach_after_tc(bfs.get_iterator().copy(false), bfs.get_source().copy(true), foreach_sb, GMTYPE_T.GMTYPE_NODEITER_ALL);
		while_sb.add_sent(foreach_out);

		// outer if
		ast_expr lev_check_out_c = ast_expr.new_comp_expr(GM_OPS_T.GMOP_EQ, ast_expr.new_field_expr(ast_field.new_field(foreach_out.get_iterator().copy(true), lev_sym.getId().copy(true))), ast_expr.new_id_expr(curr_sym.getId().copy(true)));
	...
	public static void create_user_body_main(ast_sentblock sb_to_add, ast_bfs bfs, ast_foreach out_loop, gm_symtab_entry lev_sym, gm_symtab_entry curr_sym, boolean is_fw)
	{
	...
	public static void create_fw_iteration(ast_sentblock sb, ast_bfs bfs, gm_symtab_entry lev_sym, gm_symtab_entry curr_sym, gm_symtab_entry fin_sym)
	{
		...
		ast_sentblock inner_sb = ast_sentblock.new_sentblock();
//C++ TO JAVA CONVERTER TODO TASK: Java does not have an equivalent for pointers to value types:
//ORIGINAL LINE: sbyte* inner_name = FE.voca_temp_name_and_add("_t", null, true);
		byte inner_name = GlobalMembersGm_main.FE.voca_temp_name_and_add("_t", null, true);
		ast_id inner_id = ast_id.new_id(inner_name, 0, 0);
		ast_foreach foreach_in = GlobalMembersGm_new_sents_after_tc.gm_new_foreach_after_tc(inner_id, foreach_out.get_iterator().copy(true), inner_sb, GMTYPE_T.GMTYPE_NODEITER_NBRS);
		lev_check_out_sb.add_sent(foreach_in);

		// inner level_check
		ast_expr inf = ast_expr.new_inf_expr(true);
		inf.set_type_summary(GMTYPE_T.GMTYPE_INT);
		ast_expr lev_check_in_c = ast_expr.new_comp_expr(GM_OPS_T.GMOP_EQ, ast_expr.new_field_expr(ast_field.new_field(foreach_in.get_iterator().copy(true), lev_sym.getId().copy(true))), inf);
		...
		ast_assign update_fin = ast_assign.new_assign_scala(fin_sym.getId().copy(true), update_fin_rhs, gm_assignment_t.GMASSIGN_REDUCE, foreach_out.get_iterator().copy(true), GM_REDUCE_T.GMREDUCE_AND);
	...
	public static void gm_gps_rewrite_bfs(ast_bfs b)
	{
		...
		//  }
		//
		byte[] lev_name = GlobalMembersGm_main.FE.voca_temp_name_and_add("level", null, true);
		byte[] curr_name = GlobalMembersGm_main.FE.voca_temp_name_and_add("curr_level", null, true);
//C++ TO JAVA CONVERTER TODO TASK: Java does not have an equivalent for pointers to value types:
//ORIGINAL LINE: sbyte* fin_name = FE.voca_temp_name_and_add("bfs_finished", null, true);
		byte fin_name = GlobalMembersGm_main.FE.voca_temp_name_and_add("bfs_finished", null, true);
	tangible.RefObject<String> tempRef_lev_name = new tangible.RefObject<String>(lev_name);
		gm_symtab_entry lev_sym = GlobalMembersGm_add_symbol.gm_add_new_symbol_property(sb, GMTYPE_T.GMTYPE_INT, true, b.get_source().getSymInfo(), tempRef_lev_name);
