...
public class gm_gps_gen extends gm_backend, gm_code_generator
{
	public gm_gps_gen()
	{
		super(Body);
	...
	public void do_generate_vertex_property_class(boolean is_edge_prop)
	{
		...
		{
			String.format(temp, "%s [] %s; //reverse edges (node IDs) {should this to be marshalled?}", GlobalMembersGm_main.PREGEL_BE.get_lib().is_node_type_int() ? "int" : "long", GlobalMembersGm_backend_gps.GPS_REV_NODE_ID);
			Body.pushln(temp);
		}
	...
	public void do_generate_vertex_state_body(gm_gps_basic_block b)
	{
		...
		String temp = new String(new char[1024]);
		String.format(temp, "private void _vertex_state_%d(Iterable<%s.MessageData> _msgs) {", id, GlobalMembersGm_main.FE.get_current_proc().get_procname().get_genname());
		Body.pushln(temp);
	...
	{
		if (sym.find_info_bool(GlobalMembersGm_backend_gps.GPS_FLAG_EDGE_DEFINED_INNER)) // skip edge iteration
			return;
	...
	public String get_type_string(int gm_type)
	{
		switch (gm_type)
		{
			case GMTYPE_INT:
	...
	public void generate_sent_reduce_assign(ast_assign a)
	{
		...
		if (a.is_argminmax_assign())
		{
			...
			J = rhs_list.iterator();
			for (I = lhs_list.iterator(); I.hasNext(); I++, J++)
			{
				ast_node n = I.next();
		...
		else
		{
			...
			if ((a.get_reduce_type() == GM_REDUCE_T.GMREDUCE_PLUS) || (a.get_reduce_type() == GM_REDUCE_T.GMREDUCE_MULT) || (a.get_reduce_type() == GM_REDUCE_T.GMREDUCE_AND) || (a.get_reduce_type() == GM_REDUCE_T.GMREDUCE_OR))
			{
				if (a.is_target_scalar())
					generate_rhs_id(a.get_lhs_scala());
				else
	...
	public void generate_sent_assign(ast_assign a)
	{
		...
		if (is_receiver_generate())
		{
			...
			if (a.find_info_bool(GlobalMembersGm_backend_gps.GPS_FLAG_COMM_DEF_ASSIGN)) // defined in re-write rhs
			{
				return;
			}
		}
