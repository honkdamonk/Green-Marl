...
public class gm_gps_opt_check_reverse_edges extends gm_compile_step
{
	...
	public void process(ast_procdef p)
	{
		...
		else if (T.use_in_degree())
		{
			...
			byte[] tmp_name = GlobalMembersGm_main.FE.voca_temp_name_and_add("_in_degree", null, true);
			...
			gm_symtab_entry new_prop = GlobalMembersGm_add_symbol.gm_add_new_symbol_property(sb, GMTYPE_T.GMTYPE_INT, true, T.get_target_graph(), tempRef_tmp_name);
			tmp_name = tempRef_tmp_name.argvalue;
    
			// create a routine that initializes reverse degree
			byte[] tmp_iter = GlobalMembersGm_main.FE.voca_temp_name_and_add("t");
			...
			byte[] tmp_iter2 = GlobalMembersGm_main.FE.voca_temp_name_and_add("u");
			...
			replace_in_degree_t T = new replace_in_degree_t();
			T.set_new_prop(new_prop);
			GlobalMembersGm_transform_helper.gm_replace_expr_general(p.get_body(), T);
