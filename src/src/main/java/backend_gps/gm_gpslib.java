...
//	virtual void generate_prepare_bb(gm_code_writer Body, gm_gps_basic_block b);
	...
	public void generate_vertex_prop_class_details(java.util.HashSet<gm_symtab_entry> prop, gm_code_writer Body, boolean is_edge_prop)
	{
		...
		{
			gm_symtab_entry sym = I.next();
			if (sym.find_info(GlobalMembersGm_frontend.GMUSAGE_PROPERTY) == null) // this property is set to procedure argument only
			{
				//printf("no argument property :%s\n", sym->getId()->get_genname());
				continue;
			}
			if (sym.find_info_int(GlobalMembersGm_frontend.GMUSAGE_PROPERTY) == GM_PROP_USAGE_T.GMUSAGE_IN) // Used as input only
			{
		...
		if ((is_edge_prop && prop.size() > 0) || (!is_edge_prop && proc.find_info_bool(GlobalMembersGm_backend_gps.GPS_FLAG_NODE_VALUE_INIT)))
		{
			...
			else
			{
				for (I = prop.iterator(); I.hasNext();)
				{
					...
					if ((e.find_info_int(GlobalMembersGm_frontend.GMUSAGE_PROPERTY) == GM_PROP_USAGE_T.GMUSAGE_IN) || (e.find_info_int(GlobalMembersGm_frontend.GMUSAGE_PROPERTY) == GM_PROP_USAGE_T.GMUSAGE_INOUT))
						total_count++;
				}
			}
			...
			if (total_count == 1)
			{
				...
				{
					gm_symtab_entry sym = I.next();
					if (!is_edge_prop && (sym.find_info_int(GlobalMembersGm_frontend.GMUSAGE_PROPERTY) != GM_PROP_USAGE_T.GMUSAGE_IN) && (sym.find_info_int(GlobalMembersGm_frontend.GMUSAGE_PROPERTY) != GM_PROP_USAGE_T.GMUSAGE_INOUT))
						continue;
					String name1;
					String name2;
