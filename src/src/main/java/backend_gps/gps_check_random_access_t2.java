...
public class gps_check_random_access_t2 extends gm_apply
{
	...
	public final boolean apply_lhs(ast_id i)
	{
		...
		if (sym.getType().is_node() && (sym.find_info_int(GlobalMembersGm_backend_gps.GPS_INT_SYMBOL_SCOPE) == gm_gps_new_scope_analysis_t.GPS_NEW_SCOPE_OUT))
		{
			// redefined;
			if (is_defined(sym))
			{
	...
	public final boolean apply_lhs(ast_field f)
	{
		...
		if (sym.getType().is_node_compatible())
		{
			...
			if (sym.find_info_bool(GlobalMembersGm_backend_gps.GPS_FLAG_IS_INNER_LOOP) || sym.find_info_bool(GlobalMembersGm_backend_gps.GPS_FLAG_IS_OUTER_LOOP))
			{
				// non random write
			}
			else if (sym.find_info_int(GlobalMembersGm_backend_gps.GPS_INT_SYMBOL_SCOPE) == gm_gps_new_scope_analysis_t.GPS_NEW_SCOPE_OUT)
			{
				if (s.find_info_int(GlobalMembersGm_backend_gps.GPS_INT_SYNTAX_CONTEXT) != gm_gps_new_scope_analysis_t.GPS_NEW_SCOPE_OUT)
				{
					_error = true;
