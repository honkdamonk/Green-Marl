...
//ORIGINAL LINE: void add_usage_in_BB(int bb_no, int usage, int context, int reduce_type = GMREDUCE_NULL)
	...
	public final int get_scope()
	{
		return scope;
	}
	public final void set_scope(int s)
	{
		scope = s;
	}
	...
	{
		return scope == gm_gps_scope_t.GPS_SCOPE_GLOBAL.getValue();
	}
	public final boolean is_scoped_outer()
	{
		return scope == gm_gps_scope_t.GPS_SCOPE_OUTER.getValue();
	}
	public final boolean is_scoped_inner()
	{
		return scope == gm_gps_scope_t.GPS_SCOPE_INNER.getValue();
	}
