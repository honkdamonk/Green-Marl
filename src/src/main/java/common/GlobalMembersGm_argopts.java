...
public class GlobalMembersGm_argopts
{
	...
	public static GM_comp_args[] GM_compiler_options = {new GM_comp_args(GMARGFLAG_HELP, GMARG_NULL, "Print help messages", null), new GM_comp_args(GMARGFLAG_VERSION, GMARG_NULL, "Print version", null), new GM_comp_args(GMARGFLAG_TARGET, GMARG_STRING, "Target backend. Current valid targets are 'cpp_omp', 'cpp_seq', 'gps' and 'giraph'.", "cpp_omp"), new GM_comp_args(GMARGFLAG_OUTDIR, GMARG_STRING, "Output directory.", "."), new GM_comp_args(GMARGFLAG_OPT_LEV, GMARG_INT, "Optimization Level (tobe implemented)", "1"), new GM_comp_args(GMARGFLAG_VERB_LEV, GMARG_INT, "Verbosity Level 0:silent, 1:show compile stage", "0"), new GM_comp_args(GMARGFLAG_FLIP_BFSUP, GMARG_BOOL, "Enable 'flipping edges for BFS Up-nbrs'", "1"), new GM_comp_args(GMARGFLAG_FLIP_REVERSE, GMARG_BOOL, "Enable 'flipping edges' to avoid the use of reverse edges", "0"), new GM_comp_args(GMARGFLAG_FLIP_PULL, GMARG_BOOL, "Enable 'flipping edges' to avoid the use of pull-based computation", "0"), new GM_comp_args(GMARGFLAG_MERGE_BB, GMARG_BOOL, "(For gps) Enable bb merge optimization", "1"), new GM_comp_args(GMARGFLAG_MERGE_BB_INTRA, GMARG_BOOL, "(For gps) Enable intra-loop bb merge optimization", "1"), new GM_comp_args(GMARGFLAG_STOP_STRING, GMARG_STRING, "(For debug) Stop the compiler after certain stage. <string>=(step)[.(step)]", "0"), new GM_comp_args(GMARGFLAG_REPRODUCE, GMARG_BOOL, "(For debug) When stopped, reproduce green-marl program", "1"), new GM_comp_args(GMARGFLAG_PRINTRW, GMARG_BOOL, "(For debug) When stopped, print rw analysis information", "0"), new GM_comp_args(GMARGFLAG_PRINTBB, GMARG_BOOL, "(For gps-debug) When stopped, print basicblock information", "0")};
...
//ORIGINAL LINE: int s = sizeof(GM_compiler_options) / sizeof(GM_comp_args);
