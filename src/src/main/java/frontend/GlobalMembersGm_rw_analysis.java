	...
	public static gm_range_type_t gm_get_range_from_itertype(GMTYPE_T itype)
	{
		switch (itype)
		{
			...
			case GMTYPE_COLLECTIONITER_ORDER:
				return gm_range_type_t.GM_RANGE_RANDOM; //TODO is there somthing more suitable?
			default:
...
//   false; error
//-----------------------------------------------------------
public static boolean gm_add_rwinfo_to_set(java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> info_set, gm_symtab_entry sym, gm_rwinfo new_entry)
{
...
//ORIGINAL LINE: boolean gm_add_rwinfo_to_set(java.util.HashMap<gm_symtab_entry*, java.util.LinkedList<gm_rwinfo*>*>& info_set, gm_symtab_entry* sym, gm_rwinfo* new_entry, boolean is_reduce_ops = false)
	public static boolean gm_add_rwinfo_to_set(java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> info_set, gm_symtab_entry sym, gm_rwinfo new_entry, boolean is_reduce_ops)
	{
...
//ORIGINAL LINE: i = info_set.find(sym);
		...
		{
			java.util.LinkedList<gm_rwinfo> l = new java.util.LinkedList<gm_rwinfo*>();
			l.addLast(new_entry);
			info_set.put(sym, l);
...
//ORIGINAL LINE: *e2 = *new_entry;
	...
	public static void gm_delete_rwinfo_map(java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> m)
	{
		java.util.Iterator<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> i;
	...
	// for debug
	//extern void gm_print_rwinfo_set(java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> m);
	...
	public static boolean gm_redo_rw_analysis(ast_sent s)
	{
		...
		GlobalMembersGm_traverse.gm_traverse_sents(s, RWA, GlobalMembersGm_traverse.GM_POST_APPLY); // post apply
	...
	//extern boolean gm_does_intersect(java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> S1, java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> S2, boolean regard_mutate_direction); // return true, if any of they have same symbool table

	//extern java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> gm_get_write_set(ast_sent S);
	//extern java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> gm_get_reduce_set(ast_sent S);
	//extern boolean gm_is_modified(ast_sent S, gm_symtab_entry e);
	//extern boolean gm_is_modified_with_condition(ast_sent S, gm_symtab_entry e, gm_rwinfo_query q);
...
//-----------------------------------------------------------------------------
	...
	public static boolean merge_for_if_else(java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> Target, java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> S1, java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> S2, boolean is_reduce)
	{
		boolean is_okay = true;
		java.util.Iterator<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> s1;
		java.util.Iterator<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> s2;

		// search for all elements in then-part
		for (s1 = S1.iterator(); s1.hasNext();)
		{
			gm_symtab_entry sym = s1.next().getKey();
			java.util.LinkedList<gm_rwinfo> l1 = s1.next().getValue();
...
//ORIGINAL LINE: s2 = S2.find(sym);
			...
			else
			{
				...
				for (ss1 = l1.iterator(); ss1.hasNext();)
				{
					...
					if (copy.always == true)
					{
						...
						java.util.LinkedList<gm_rwinfo> l2 = s2.next().getValue();
	...
	public static boolean is_reduce_error(gm_rwinfo old, gm_rwinfo neo)
	{
		...
		if (old.reduce_op != neo.reduce_op)
		{
			...
			GlobalMembersGm_error.gm_type_error(GM_ERRORS_AND_WARNINGS.GM_ERROR_DOUBLE_BOUND_OP, neo.location.get_line(), neo.location.get_col(), GlobalMembersGm_misc.gm_get_reduce_string(old.reduce_op));
			return true;
		}
		...
		{
			// generate error message
			GlobalMembersGm_error.gm_type_error(GM_ERRORS_AND_WARNINGS.GM_ERROR_DOUBLE_BOUND_ITOR, neo.location.get_line(), neo.location.get_col(), old.bound_symbol.getId().get_orgname());
			return true;
		}

		return false;
	}
	public static java.util.HashMap<gm_symtab_entry, range_cond_t> Default_DriverMap = new java.util.HashMap<gm_symtab_entry, range_cond_t>();
public static void traverse_expr_for_readset_adding(ast_expr e, java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> rset)
{
	traverse_expr_for_readset_adding(e, rset, Default_DriverMap);
}
//C++ TO JAVA CONVERTER NOTE: Java does not allow default values for parameters. Overloaded methods are inserted above.
//ORIGINAL LINE: void traverse_expr_for_readset_adding(ast_expr* e, java.util.HashMap<gm_symtab_entry*, java.util.LinkedList<gm_rwinfo*>*>& rset, java.util.HashMap<gm_symtab_entry*, range_cond_t>& DrvMap = Default_DriverMap)
	public static void traverse_expr_for_readset_adding(ast_expr e, java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> rset, java.util.HashMap<gm_symtab_entry, range_cond_t> DrvMap)
	{
		...
		switch (e.get_opclass())
		{
			...
			case GMEXPR_TER:
			{
				...
				java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> R1 = new java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>>();
				GlobalMembersGm_rw_analysis.traverse_expr_for_readset_adding(e.get_left_op(), R1, DrvMap);
				java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> R2 = new java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>>();
	...
	public static void traverse_expr_for_readset_adding_field(ast_expr e, java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> rset, java.util.HashMap<gm_symtab_entry, range_cond_t> DrvMap)
	{
	...
	public static void traverse_expr_for_readset_adding_builtin(ast_expr_builtin builtin, java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> rset, java.util.HashMap<gm_symtab_entry, range_cond_t> DrvMap)
	{
		// add every arguments in the readset
		java.util.LinkedList<ast_expr> args = builtin.get_args();
	...
	public static void traverse_expr_for_readset_adding_foreign(ast_expr_foreign f, java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> rset, java.util.HashMap<gm_symtab_entry, range_cond_t> DrvMap)
	{

		gm_rwinfo new_entry;
		java.util.LinkedList<ast_node> N = f.get_parsed_nodes();
	...
	public static void traverse_expr_for_readset_adding_reduce(ast_expr_reduce e2, java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> rset, java.util.HashMap<gm_symtab_entry, range_cond_t> DrvMap)
	{
		...
		boolean is_conditional = (f != null) || GlobalMembersGm_defs.gm_is_collection_iter_type(iter_type);
...
//C++ TO JAVA CONVERTER NOTE: Java has no need of forward class declarations:
	...
	public static boolean merge_for_if(java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> Target, java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> S1, boolean is_reduce)
	{
		boolean is_okay = true;
		java.util.Iterator<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> s1;
	...
	// return: is_okay
	//-----------------------------------------------------------------------------
	public static boolean merge_for_sentblock(ast_sentblock s, java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> target, java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> old, boolean is_reduce)
	{
		boolean is_okay = true;
		java.util.Iterator<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> i;
	...
	// return: is_okay
	//-----------------------------------------------------------------------------
	public static boolean merge_all(java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> target, java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> old, boolean is_reduce)
	{
		boolean is_okay = true;
		java.util.Iterator<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> i;
		for (i = old.iterator(); i.hasNext();)
	...
	public static boolean merge_body(java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> R, java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> W, java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> D, java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> M, ast_sent s, boolean is_conditional) // body sentence -  top
	{
		gm_rwinfo_sets sets2 = GlobalMembersGm_rw_analysis.get_rwinfo_sets(s);
//C++ TO JAVA CONVERTER WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: java.util.HashMap<gm_symtab_entry*, java.util.LinkedList<gm_rwinfo*>*>& R2 = sets2->read_set;
		java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> R2 = new java.util.HashMap(sets2.read_set);
//C++ TO JAVA CONVERTER WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: java.util.HashMap<gm_symtab_entry*, java.util.LinkedList<gm_rwinfo*>*>& W2 = sets2->write_set;
		java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> W2 = new java.util.HashMap(sets2.write_set);
//C++ TO JAVA CONVERTER WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: java.util.HashMap<gm_symtab_entry*, java.util.LinkedList<gm_rwinfo*>*>& D2 = sets2->reduce_set;
		java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> D2 = new java.util.HashMap(sets2.reduce_set);
//C++ TO JAVA CONVERTER WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: java.util.HashMap<gm_symtab_entry*, java.util.LinkedList<gm_rwinfo*>*>& M2 = sets2->mutate_set;
		java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> M2 = new java.util.HashMap(sets2.mutate_set);
		boolean is_okay = true;

		if (!is_conditional)
		{
			is_okay = GlobalMembersGm_rw_analysis.merge_all(R, R2, false) && is_okay; // copy as is
		...
		{
			is_okay = GlobalMembersGm_rw_analysis.merge_for_if(R, R2, false) && is_okay; // copy and change it as conditional
	...
	//-----------------------------------------------------------------------------
	//
	public static boolean cleanup_iterator_access(ast_id iter, java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> T_temp, java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> T, int iter_type, boolean is_parallel)
	{
		boolean is_okay = true;
		gm_symtab_entry iter_sym = iter.getSymInfo();
		java.util.Iterator<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> i;
		int range = GlobalMembersGm_rw_analysis.gm_get_range_from_itertype(iter_type);
		...
		for (i = T_temp.iterator(); i.hasNext();)
		{
			...
			for (ii = l.iterator(); ii.hasNext();)
			{
				...
				{
					if (cp.access_range == gm_range_type_t.GM_RANGE_SINGLE.getValue())
					{
						// scalar, do nothing
					}
					else if (is_parallel)
					{
						...
						cp.access_range = gm_range_type_t.GM_RANGE_RANDOM.getValue();
						cp.driver = null;
					}
				}
				else if (is_parallel)
	...
	public static void cleanup_iterator_access_bfs(java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> T)
	{
		// bfs iter ==> conditional, linear iteration
		boolean new_always = false;
		int new_range = gm_range_type_t.GM_RANGE_LINEAR.getValue(); // G.Nodes or G.Edges
		...
		for (i = T.iterator(); i.hasNext();)
		{
			...
			{
				gm_rwinfo e = ii.next();
				if ((e.access_range == gm_range_type_t.GM_RANGE_LEVEL.getValue()) || (e.access_range == gm_range_type_t.GM_RANGE_LEVEL_UP.getValue()) || (e.access_range == gm_range_type_t.GM_RANGE_LEVEL_DOWN.getValue()))
				{
					is_target = true;
					location = e.location;
//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent to the STL list 'erase' method in Java:
				...
				else if ((e.access_range == gm_range_type_t.GM_RANGE_LINEAR.getValue()) && (e.always == false))
				{
					is_already = true;
				}
			}
	...
	public static boolean cleanup_iterator_access_reduce(ast_id iter, java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> D_temp, java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> D, java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> W, java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> B, int iter_type, boolean is_parallel) // Nodes or NBRS -  bound-set for Foreach -  write -  reduce map of the Foreach-statement -  reduce map of the body
			...
			for (ii = l.iterator(); ii.hasNext();)
			{
				...
				else if (is_parallel)
				{
					if (cp.driver == null)
					{
						if (cp.access_range != gm_range_type_t.GM_RANGE_SINGLE.getValue())
						{
							cp.access_range = gm_range_type_t.GM_RANGE_RANDOM.getValue(); // scalar access becomes random access
						}
					}
				...
				if (cp.bound_symbol == iter_sym)
				{
					...
					cp.reduce_op = GM_REDUCE_T.GMREDUCE_NULL.getValue();
