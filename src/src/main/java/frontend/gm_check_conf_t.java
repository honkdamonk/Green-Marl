...
public class gm_check_conf_t extends gm_apply
{
	...
	public boolean apply(ast_sent s)
	{
		if (s.get_nodetype() == AST_NODE_TYPE.AST_FOREACH)
		{
			...
			java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> R = GlobalMembersGm_rw_analysis.get_rwinfo_sets(fe).read_set; // body + filter
			java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> W = GlobalMembersGm_rw_analysis.get_rwinfo_sets(body).write_set;
			java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> D = GlobalMembersGm_rw_analysis.get_rwinfo_sets(body).reduce_set;
			java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> M = GlobalMembersGm_rw_analysis.get_rwinfo_sets(body).mutate_set;

			GlobalMembersGm_rw_analysis_check2.check_rw_conf_error(R, W, gm_conflict_t.RW_CONFLICT, Report); // R-W (warning)
			GlobalMembersGm_rw_analysis_check2.check_rw_conf_error(W, W, gm_conflict_t.WW_CONFLICT, Report); // W-W (warning)
			GlobalMembersGm_rw_analysis_check2.check_rw_conf_error(R, M, gm_conflict_t.RM_CONFLICT, Report); // R-M (warning)
			GlobalMembersGm_rw_analysis_check2.check_rw_conf_error(M, M, gm_conflict_t.MM_CONFLICT, Report); // M-M (warning)
		...
		else if (s.get_nodetype() == AST_NODE_TYPE.AST_BFS)
		{
			...
			java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> R_filter = new java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>>();
			...
			if (bfs.get_fbody() != null)
			{
				...
				java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> R = GlobalMembersGm_rw_analysis.get_rwinfo_sets(body).read_set;
				java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> W = GlobalMembersGm_rw_analysis.get_rwinfo_sets(body).write_set;
				java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> D = GlobalMembersGm_rw_analysis.get_rwinfo_sets(body).reduce_set;
				java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> M = GlobalMembersGm_rw_analysis.get_rwinfo_sets(body).mutate_set;

				GlobalMembersGm_rw_analysis_check2.check_rw_conf_error(R, W, gm_conflict_t.RW_CONFLICT, Report); // R-W (warning)
				GlobalMembersGm_rw_analysis_check2.check_rw_conf_error(R_filter, W, gm_conflict_t.RW_CONFLICT, Report); // R-W (warning)
				GlobalMembersGm_rw_analysis_check2.check_rw_conf_error(W, W, gm_conflict_t.WW_CONFLICT, Report); // W-W (warning)
				GlobalMembersGm_rw_analysis_check2.check_rw_conf_error(R, M, gm_conflict_t.RM_CONFLICT, Report); // R-M (warning)
				GlobalMembersGm_rw_analysis_check2.check_rw_conf_error(M, M, gm_conflict_t.MM_CONFLICT, Report); // M-M (warning)
				is_okay = is_okay && GlobalMembersGm_rw_analysis_check2.check_rw_conf_error(R, D, gm_conflict_t.RD_CONFLICT, Report); // R-D
	...
	private java.util.LinkedList<conf_info_t> Report = new java.util.LinkedList<conf_info_t>();
