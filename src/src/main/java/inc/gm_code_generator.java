...
public abstract class gm_code_generator
{
	...
	public void generate_expr_foreign(ast_expr e)
	{
		...
		java.util.LinkedList< String > T = f.get_parsed_text();
	...
	public abstract String get_type_string(int prim_t); // returned string should be copied before usage.
	...
	public void generate_expr_val(ast_expr e)
	{
		...
		switch (e.get_opclass())
		{
			...
			case GMEXPR_FVAL:
				...
				{
					String.format(temp, "(float)(%lf)", e.get_fval()); // to be changed
					_Body.push(temp);
				}
				else
				{
