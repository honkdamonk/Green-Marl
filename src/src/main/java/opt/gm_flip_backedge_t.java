...
//	;
	...
	public boolean apply(ast_sent sent)
	{
		...
		java.util.LinkedList<ast_sent> S = body.get_sents();

		java.util.LinkedList<gm_symtab_entry> targets = new java.util.LinkedList<gm_symtab_entry>();
		java.util.HashMap<gm_symtab_entry, Boolean> check_init = new java.util.HashMap<gm_symtab_entry, Boolean>();
		...
		for (I = S.iterator(); I.hasNext();)
		{
			...
			if (iter_type != GMTYPE_T.GMTYPE_NODEITER_UP_NBRS.getValue())
				return true;
		...
		for (I = (sb.get_sents()).begin(); I.hasNext();)
		{
			ast_sent s = I.next();
			gm_rwinfo_sets RW = GlobalMembersGm_rw_analysis.gm_get_rwinfo_sets(s);
//C++ TO JAVA CONVERTER WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: java.util.HashMap<gm_symtab_entry*, java.util.LinkedList<gm_rwinfo*>*>& W = RW->write_set;
			java.util.HashMap<gm_symtab_entry, java.util.LinkedList<gm_rwinfo>> W = new java.util.HashMap(RW.write_set);
			...
			for (T = targets.iterator(); T.hasNext();)
			{
				...
				if (!W.containsKey(t))
					continue;
				java.util.LinkedList<gm_rwinfo> lst = W.get(t);
	...
	public final void flip_edges(ast_assign a, ast_sentblock p)
	{
		...
		int new_iter_type = GMTYPE_T.GMTYPE_NODEITER_DOWN_NBRS.getValue();
		...
		ast_assign new_assign = ast_assign.new_assign_field(new_lhs, new_rhs, gm_assignment_t.GMASSIGN_REDUCE, bfs.get_iterator().copy(true), old_rhs.get_reduce_type());
		...
		GlobalMembersGm_transform_helper.gm_ripoff_sent(a, GlobalMembersGm_transform_helper.GM_NOFIX_SYMTAB); // no need to fix symtab for a -- it will be deleted.
		if (a != null)
			a.dispose();
	}

	private java.util.LinkedList<ast_sentblock> _tops = new java.util.LinkedList<ast_sentblock>();
	private java.util.LinkedList<ast_assign> _cands = new java.util.LinkedList<ast_assign>();
}
